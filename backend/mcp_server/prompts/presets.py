...
def family_overview() -> str:
    """家庭资产全面检视 — 四桶资产总览，分析配置合理性，检查保障与到期提醒。"""
    return (
        "请帮我做一次家庭资产全面检视。请按以下步骤操作：\n\n"
        "1. 调用 get_family_asset_summary 获取四桶资产总览（活钱、稳钱、长钱、保险）\n"
        "2. 调用 get_liquid_assets 查看活钱明细\n"
        "3. 调用 get_stable_assets 查看稳钱明细\n"
        "4. 调用 get_insurance_policies 查看保险保障情况\n"
        "5. 调用 get_portfolio_summary 查看长钱（基金组合）表现\n\n"
        "然后基于以上数据，给出全面分析：\n"
        "- **四桶配置评估**: 活钱、稳钱、长钱的比例是否合理？"
        "一般建议活钱占 10-20%（3-6 个月生活费），稳钱占 20-40%，长钱占 40-60%\n"
        "- **流动性分析**: 活钱是否足够覆盖紧急需求？利率是否有优化空间？\n"
        "- **稳钱到期管理**: 是否有即将到期的定期存款或理财产品？到期后如何安排？\n"
        "- **保障缺口分析**: 家庭成员的保险覆盖是否充分？"
        "重疾、医疗、意外、寿险四大险种是否齐全？保额是否足够？\n"
        "- **待办提醒**: 汇总所有需要关注的到期日和续费日\n"
        "- **优化建议**: 综合以上分析，给出 2-3 条具体可操作的改善建议\n"
    )
